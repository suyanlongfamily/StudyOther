...

{
	...
	public:
		...
		virtual const char *what() const throw()
		{
			static char buf[256];
			memset(buf, 0, 256);
			sprintf_s(buf, "bad lexical cast: from  %s to %s", source->name(), target->name());
			//sprintf(buf, "bad lexical cast: from  %s to %s", source->name(), target->name());
			buf[255] = '\0';
			return buf;
		}
...
*/
			...
			{
				return stream >> output && (stream >> std::ws).eof();
			}
	...
	template<typename Target, typename Source>
	Target lex_cast(const Source &arg)
	{
